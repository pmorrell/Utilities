...
readarray -t CORE_genes < ${CORE}
readarray -t NONCORE_genes < ${NONCORE}
...
for i in "${CORE_genes[@]}"
    do
    grep -E "${i}" <(echo "${GENES}") | cut -f 1,4,5 | sort -k1,1 -k2,2n -k3,3n >${OUT_DIR}/CORE_POS.txt
done

for j in "${NONCORE_genes[@]}"
    do
    grep -E "${j}" <(echo "${GENES}") | cut -f 1,4,5| sort -k1,1 -k2,2n -k3,3n >${OUT_DIR}/NONCORE_POS.txt
  done
